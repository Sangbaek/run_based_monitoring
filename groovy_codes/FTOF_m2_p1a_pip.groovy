...
def grtl2 = (1..6).collect{
  def gr = new GraphErrors('sec'+it)
  gr.setTitle("FTOF p1a #pi^+ mass^2 sigma")
  gr.setTitleY("FTOF p1a #pi^+ mass^2 sigma (GeV^2)")
  gr.setTitleX("run number")
  return gr
}
...
for(arg in args) {
  ...
  (0..<6).each{
    ...
    grtl2it].addPoint(run, f1.getParameter(2), 0, 0)
    out.addDataSet(h1)
    out.addDataSet(f1)
    out2.addDataSet(h1)
    out2.addDataSet(f1)
...
out.writeFile('FTOF_m2_p1a_pip_mean.hipo')
out2.mkdir('/timelines')
out2.cd('/timelines')
grtl2.each{ out2.addDataSet(it) }
out2.writeFile('FTOF_m2_p1a_pip_sigma.hipo')
