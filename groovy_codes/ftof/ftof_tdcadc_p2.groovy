...
def grtl = (1..6).collect{
  ...
  gr.setTitle("p2 t_tdc-t_fadc (mean)")
  gr.setTitleY("p2 t_tdc-t_fadc (mean) (ns)")
  gr.setTitleX("run number")
  return gr
}
def grtl2 = (1..6).collect{
  def gr = new GraphErrors('Sigma,'+it)
  gr.setTitle("p2 t_tdc-t_fadc (sigma)")
  gr.setTitleY("p2 t_tdc-t_fadc (sigma) (ns)")
  gr.setTitleX("run number")
  return gr
}
...
out.writeFile('ftof_tdcadc_time_p2_mean.hipo')

out2.mkdir('/timelines')
out2.cd('/timelines')
grtl2.each{ out2.addDataSet(it) }
out2.writeFile('ftof_tdcadc_time_p2_sigma.hipo')
...
private void initTimeGaussFitPar(F1D f1, H1F h1) {
        ...
        // double pm = hRMS;
        f1.setRange(hMean-2*hRMS, hMean+2*hRMS);
