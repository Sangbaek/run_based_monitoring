...
def grtl = (1..6).collect{
  ...
  gr.setTitle("p1b t_tdc-t_fadc (mean)")
  gr.setTitleY("p1b t_tdc-t_fadc (mean) (ns)")
  gr.setTitleX("run number")
  return gr
}
def grtl2 = (1..6).collect{
  def gr = new GraphErrors('Sigma,'+it)
  gr.setTitle("p1b t_tdc-t_fadc (sigma)")
  gr.setTitleY("p1b t_tdc-t_fadc (sigma) (ns)")
  gr.setTitleX("run number")
  return gr
}
...
out.writeFile('ftof_tdcadc_time_p1b_mean.hipo')

out2.mkdir('/timelines')
out2.cd('/timelines')
grtl2.each{ out2.addDataSet(it) }
out2.writeFile('ftof_tdcadc_time_p1b_sigma.hipo')
