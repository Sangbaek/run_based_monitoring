...
for (s = 0; s <6; s++) {
  for (r = 0; r <4; r++) {
    ...
    int counter = r + 4*s
    def h1 = dir.getObject(String.format('/HTCC/H_HTCC_nphe_s%d_r%d_side0',s+1,r+1)) //left
    def h2 = dir.getObject(String.format('/HTCC/H_HTCC_nphe_s%d_r%d_side1',s+1,r+1)) //right
    h1.Add(h2)
